// 생성한 이야기 데이터 연동 예제 (MyCreatedStories 컴포넌트)
import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { Button, Row, Col, Card, Modal, Form } from 'react-bootstrap';

const MyCreatedStories = () => {
  const [stories, setStories] = useState([]); // 백엔드 연동된 이야기 리스트
  const [showModal, setShowModal] = useState(false);
  const [selectedStory, setSelectedStory] = useState(null);

  useEffect(() => {
    // 사용자 ID가 필요하다면 백엔드 세션 또는 로컬스토리지 등에서 불러와야 함
    axios.get('http://localhost:8080/api/stories/mine')
      .then(res => setStories(res.data))
      .catch(err => console.error('내 이야기 불러오기 실패:', err));
  }, []);
